	   ...
	   <center>
			<table border="1" cellpadding="0" cellspacing="0" width="100%">
				 <tr>
				  ...
				  <td rowspan="1" style="font-size: 11px; padding-left: 5px;">CODIGO: 201-300-ACTCOM{{ $actividad_complementaria->plan_trabajo->tercero->id_licencia }}-FOR{{ $actividad_complementaria->id_actividad_complementaria }}</td>
				 </tr>
				 <tr>
...
<div style="text-align: right;"><b style="font-size: 14px;">PERIODO LECTIVO {{ $actividad_complementaria->plan_trabajo->periodo_academico->periodo }}</b></div>
<table class="tabla_1" border="1" cellpadding="0" cellspacing="0">
	<tr style="background-color: #ccffcc">
		<td colspan="16"><p>APELLIDOS Y NOMBRES: {{ strtoupper($actividad_complementaria->plan_trabajo->tercero->apellido) }} {{ strtoupper($actividad_complementaria->plan_trabajo->tercero->nombre) }} </p></td>
	</tr>
	<tr>
		<td colspan="4" style="width: 25% !important"><b>INFORME NUMERO: </b> </td>
		<td colspan="12">{{ $corte }}</td>
	</tr>
	<tr>
		<td colspan="4" style="width: 25% !important"><b>FECHA: </b> </td>
		<td colspan="12">{{ $actividad_complementaria->get_ultima_fecha_actualizada($actividad_plan_trabajo->id_dominio_tipo) }}</td>
	</tr>
	<tr>
		<td colspan="4" style="width: 25% !important"><b>TIPO DE ACTIVIDAD: </b> </td>
		<td colspan="12">{{ $actividad_plan_trabajo->tipo->dominio }}</td>
	</tr>
	<tr>
		<td colspan="4" style="width: 25% !important"><b>NOMBRE DE LA ACTIVIDAD: </b> </td>
		<td colspan="12"><b>{{ strtoupper($actividad_plan_trabajo->nombre) }}</b></td>
	</tr>
	<tr style="background-color: #ccffcc">
...
informe, registrando la respectiva fecha de realización y el tipo de evidencia)</td>
	...
	@foreach ($actividad_complementaria->detalles as $detalle)
		@if ($detalle->id_actividad_plan_trabajo == $actividad_plan_trabajo->id_actividad_plan_trabajo)
			
			<tr>
				<th colspan="1" style="font-size: 12px;"><center>{{ $cont }}</center></th>
				<td colspan="11">{{ $detalle->descripcion }}</td>
				<td colspan="2"><center>{{ $detalle->evidencia }} @if ($detalle->link_evidencia and $detalle->link_evidencia != "")
					<br>
					<a  href="{{ $detalle->link_evidencia }}"> Anexo </a>
				@endif </center></td>
				<td colspan="2"><center>{{ $detalle->fecha_evidencia }}</center></td>
